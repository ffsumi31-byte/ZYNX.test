...
-- move your mouse while tracking to temporarily pause tracking.
-- after 1 second of no mouse movement, tracking resumes automatically.
local MOUSE_RELEASE_DELAY = 1
local MOUSE_DELTA_THRESHOLD = 2

local PANEL_WIDTH = 290

local CLOSED_HEIGHT = 280
local OPEN_HEIGHT = 384

local CLOSED_TARGET_Y = 147
local OPEN_TARGET_Y = 251

local CLOSED_BUTTON_Y = 187
local OPEN_BUTTON_Y = 291

local CLOSED_HINT_Y = 232
local OPEN_HINT_Y = 336
...
local manualCameraOverride = false
...
userInput.Size = UDim2.new(1, -24, 0, 34)
userInput.Position = UDim2.fromOffset(12, 100)
userInput.BackgroundColor3 = Color3.fromRGB(25, 22, 33)
userInput.BorderSizePixel = 0
userInput.Text = ""
userInput.PlaceholderText = "TYPE USER / DISPLAY NAME + ENTER"
userInput.PlaceholderColor3 = Color3.fromRGB(112, 105, 128)
userInput.TextColor3 = Color3.fromRGB(225, 218, 238)
...
local userInputStroke = Instance.new("UIStroke")
userInputStroke.Thickness = 1
userInputStroke.Color = Color3.fromRGB(70, 55, 92)
userInputStroke.Transparency = 0.45
userInputStroke.Parent = userInput
...
playerList.Position = UDim2.fromOffset(12, 140)
...
local function updateCamera(dt)
	...
	if manualCameraOverride then
		if tick() - lastMouseMoveTime < MOUSE_RELEASE_DELAY then
			return
		end
...
local function createPlayerButton(targetPlayer)
	...
	button.MouseButton1Click:Connect(function()
		...
		-- AUTO START TRACKING AFTER PLAYER SELECTION
		startTracking()
	end)
end
...
local function findPlayerByText(text)
	...
	for _, targetPlayer in ipairs(Players:GetPlayers()) do
		if targetPlayer ~= player then
			if string.lower(targetPlayer.Name) == query
				or string.lower(targetPlayer.DisplayName) == query then
				return targetPlayer
			end
		end
	end

	-- then allow partial matches for faster testing
	for _, targetPlayer in ipairs(Players:GetPlayers()) do
		if targetPlayer ~= player then
			local username = string.lower(targetPlayer.Name)
			local display = string.lower(targetPlayer.DisplayName)

			if string.sub(username, 1, #query) == query
				or string.sub(display, 1, #query) == query then
				return targetPlayer
			end
		end
	end
...
local function quickTrackUser()
	local targetPlayer = findPlayerByText(userInput.Text)

	if not targetPlayer then
		targetStatus.Text = "TARGET  •  NOT FOUND"
		targetStatus.TextColor3 = Color3.fromRGB(255, 125, 145)

		tween(userInputStroke, 0.1, {
			Color = Color3.fromRGB(220, 70, 110),
			Transparency = 0.05
		})

		task.delay(0.3, function()
			tween(userInputStroke, 0.15, {
				Color = Color3.fromRGB(70, 55, 92),
				Transparency = 0.45
			})
		end)
	...
	userInput.Text = targetPlayer.Name
...
userInput.FocusLost:Connect(function(pressedEnter)
...
userInput.Focused:Connect(function()
	tween(userInputStroke, 0.1, {
		Color = Color3.fromRGB(165, 95, 255),
		Transparency = 0.05
	})
end)

userInput.FocusLost:Connect(function()
	tween(userInputStroke, 0.12, {
		Color = Color3.fromRGB(70, 55, 92),
		Transparency = 0.45
	})
end)
...
UserInputService.InputChanged:Connect(function(inputObject)
	if inputObject.UserInputType == Enum.UserInputType.MouseMovement
		...
		if inputObject.Delta.Magnitude > MOUSE_DELTA_THRESHOLD then
