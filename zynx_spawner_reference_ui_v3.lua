-- ZYNX SPawner Reference UI V3
-- Built from the last working ZYNX PLAYER/TRADE base.
-- PETS uses the uploaded spawner's UI/interaction ideas while remaining local-visual only.
-- No ClientData prediction, RouterClient replacement, or game inventory mutation.
...
local function applyVisualPetStyle(model, baseColor)
	local neonOn = petState.Neon == true
	local megaOn = petState.Mega == true
...
local function createLocalVisualPet(petName)
	...
	model:SetAttribute("Neon", petState.Neon == true)
	model:SetAttribute("Mega", petState.Mega == true)
	model:SetAttribute("Fly", petState.Fly == true)
	model:SetAttribute("Ride", petState.Ride == true)
...
spawnPet.MouseButton1Click:Connect(function()
	...
	local states = {}
	if petState.Neon then table.insert(states, "Neon") end
	if petState.Mega then table.insert(states, "Mega") end
	if petState.Fly then table.insert(states, "Fly") end
	if petState.Ride then table.insert(states, "Ride") end
