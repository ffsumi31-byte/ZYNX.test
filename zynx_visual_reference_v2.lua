...
local function zynxTween(obj, props, duration, style, direction)
...
--==================================================
-- ZYNX PATCH: LOCAL VISUAL PET RENDERER
-- Self-contained visual models only; no inventory/remotes.
--==================================================

local spawnedVisualPets = spawnedVisualPets or {}
local visualPetSerial = 0

local PET_COLORS = {
	["Shadow Dragon"] = Color3.fromRGB(45, 35, 70),
	["Frost Dragon"] = Color3.fromRGB(175, 235, 255),
	["Bat Dragon"] = Color3.fromRGB(120, 70, 40),
	["Giraffe"] = Color3.fromRGB(235, 185, 75),
	["Owl"] = Color3.fromRGB(145, 95, 60),
	["Parrot"] = Color3.fromRGB(220, 55, 55),
	["Crow"] = Color3.fromRGB(35, 35, 42),
	["Evil Unicorn"] = Color3.fromRGB(65, 40, 80),
	["Arctic Reindeer"] = Color3.fromRGB(225, 235, 245),
	["Turtle"] = Color3.fromRGB(80, 155, 95),
	["Kangaroo"] = Color3.fromRGB(175, 120, 75),
	["Unicorn"] = Color3.fromRGB(245, 220, 245),
}

local function zynxPetPart(parent, name, size, color, cf, shape)
...
local function applyVisualPetStyle(model, baseColor)
	local neonOn = neonToggle and neonToggle.Text == "ON"
	local megaOn = megaToggle and megaToggle.Text == "ON"
	...
	if megaOn then
		task.spawn(function()
			...
			while model.Parent do
				...
				for _, obj in ipairs(model:GetDescendants()) do
					if obj:IsA("BasePart") then
						obj.Color = Color3.fromHSV(hue, 0.7, 1)
						obj.Material = Enum.Material.Neon
					end
				end
				task.wait(0.04)
		...
		for _, obj in ipairs(model:GetDescendants()) do
			if obj:IsA("BasePart") and obj.Name ~= "Eye" then
				obj.Color = baseColor
			end
		end
	end
...
local function createLocalVisualPet(petName)
	visualPetSerial += 1

	local model = Instance.new("Model")
	model.Name = "ZYNX_VisualPet_" .. tostring(visualPetSerial) .. "_" .. petName:gsub("%s+", "_")
	model:SetAttribute("ZYNXVisualPet", true)
	model:SetAttribute("PetName", petName)
	model:SetAttribute("Neon", neonToggle and neonToggle.Text == "ON")
	model:SetAttribute("Mega", megaToggle and megaToggle.Text == "ON")
	model:SetAttribute("Fly", flyToggle and flyToggle.Text == "ON")
	model:SetAttribute("Ride", rideToggle and rideToggle.Text == "ON")

	local root = player.Character and player.Character:FindFirstChild("HumanoidRootPart")
	local origin = root and (root.CFrame * CFrame.new(4, 0.25, -4)) or CFrame.new(0, 4, 0)
	local base = PET_COLORS[petName] or Color3.fromRGB(160, 110, 210)

	local body = zynxPetPart(model, "Body", Vector3.new(2.8, 1.8, 3.4), base, origin, Enum.PartType.Ball)
	local head = zynxPetPart(model, "Head", Vector3.new(2.15, 2.0, 2.0), base, origin * CFrame.new(0, 0.45, -2.0), Enum.PartType.Ball)
	zynxPetPart(model, "EarL", Vector3.new(0.65, 1.0, 0.55), base, head.CFrame * CFrame.new(-0.65, 0.9, 0), Enum.PartType.Ball)
	zynxPetPart(model, "EarR", Vector3.new(0.65, 1.0, 0.55), base, head.CFrame * CFrame.new(0.65, 0.9, 0), Enum.PartType.Ball)

	local eyeColor = Color3.fromRGB(20, 20, 24)
	local eyeL = zynxPetPart(model, "Eye", Vector3.new(0.22, 0.30, 0.18), eyeColor, head.CFrame * CFrame.new(-0.43, 0.18, -0.94), Enum.PartType.Ball)
	local eyeR = zynxPetPart(model, "Eye", Vector3.new(0.22, 0.30, 0.18), eyeColor, head.CFrame * CFrame.new(0.43, 0.18, -0.94), Enum.PartType.Ball)
	eyeL.Material = Enum.Material.Neon
	eyeR.Material = Enum.Material.Neon

	zynxPetPart(model, "LegFL", Vector3.new(0.65, 1.2, 0.65), base, body.CFrame * CFrame.new(-0.75, -0.95, -0.85), Enum.PartType.Ball)
	zynxPetPart(model, "LegFR", Vector3.new(0.65, 1.2, 0.65), base, body.CFrame * CFrame.new(0.75, -0.95, -0.85), Enum.PartType.Ball)
	zynxPetPart(model, "LegBL", Vector3.new(0.65, 1.2, 0.65), base, body.CFrame * CFrame.new(-0.75, -0.95, 0.85), Enum.PartType.Ball)
	zynxPetPart(model, "LegBR", Vector3.new(0.65, 1.2, 0.65), base, body.CFrame * CFrame.new(0.75, -0.95, 0.85), Enum.PartType.Ball)
	zynxPetPart(model, "Tail", Vector3.new(0.55, 0.55, 1.65), base, body.CFrame * CFrame.new(0, 0.25, 2.15) * CFrame.Angles(math.rad(-25), 0, 0), Enum.PartType.Ball)
	...
	applyVisualPetStyle(model, base)

	-- Local follow loop. Fly raises the pet; Ride is stored/displayed as visual state.
	task.spawn(function()
		local t = math.random() * 6
		while model.Parent do
			local charRoot = player.Character and player.Character:FindFirstChild("HumanoidRootPart")
			if charRoot and model.PrimaryPart then
				t += 0.035
				local flyHeight = model:GetAttribute("Fly") and 3.0 or 0.15
				local bob = math.sin(t * 2.2) * (model:GetAttribute("Fly") and 0.35 or 0.12)
				local target = charRoot.CFrame * CFrame.new(3.8, flyHeight + bob, 3.2)
				model:PivotTo(model:GetPivot():Lerp(target, 0.12))
			end
			task.wait()
		end
	end)

	spawnedVisualPets[model] = true
	return model
end

spawnPet.MouseButton1Click:Connect(function()
	if not selectedPet then
		petStatus.Text = "Select a pet first."
		petStatus.TextColor3 = RED
		return
	end

	local ok, modelOrError = pcall(function()
		return createLocalVisualPet(selectedPet)
	end)

	if not ok then
		petStatus.Text = "Visual spawn failed."
		petStatus.TextColor3 = RED
		warn("[ZYNX PETS] " .. tostring(modelOrError))
		return
	end

	local states = {}
	if neonToggle and neonToggle.Text == "ON" then table.insert(states, "Neon") end
	if megaToggle and megaToggle.Text == "ON" then table.insert(states, "Mega") end
	if flyToggle and flyToggle.Text == "ON" then table.insert(states, "Fly") end
	if rideToggle and rideToggle.Text == "ON" then table.insert(states, "Ride") end

	local suffix = #states > 0 and (" [" .. table.concat(states, ", ") .. "]") or ""
	petStatus.Text = "Spawned " .. selectedPet .. suffix
	petStatus.TextColor3 = GREEN
end)
...
--==================================================
-- ZYNX V2: LOCAL TRADE VISUAL SEQUENCE
-- Reference-inspired TweenService presentation only.
--==================================================
local tradeVisualBusy = false

local tradeOverlay = Instance.new("Frame")
tradeOverlay.Name = "ZYNXTradeOverlay"
tradeOverlay.Size = UDim2.fromScale(1, 1)
tradeOverlay.BackgroundColor3 = Color3.fromRGB(8, 8, 12)
tradeOverlay.BackgroundTransparency = 1
tradeOverlay.Visible = false
tradeOverlay.ZIndex = 200
tradeOverlay.Parent = main

local tradeResultCard = Instance.new("Frame")
tradeResultCard.AnchorPoint = Vector2.new(0.5, 0.5)
tradeResultCard.Position = UDim2.fromScale(0.5, 0.5)
tradeResultCard.Size = UDim2.fromOffset(300, 132)
tradeResultCard.BackgroundColor3 = Color3.fromRGB(18, 18, 25)
tradeResultCard.BackgroundTransparency = 1
tradeResultCard.ZIndex = 201
tradeResultCard.Parent = tradeOverlay

local tradeResultCorner = Instance.new("UICorner")
tradeResultCorner.CornerRadius = UDim.new(0, 16)
tradeResultCorner.Parent = tradeResultCard

local tradeResultStroke = Instance.new("UIStroke")
tradeResultStroke.Color = Color3.fromRGB(125, 72, 235)
tradeResultStroke.Thickness = 1.2
tradeResultStroke.Transparency = 1
tradeResultStroke.Parent = tradeResultCard

local tradeCheck = Instance.new("TextLabel")
tradeCheck.Size = UDim2.fromOffset(48, 48)
tradeCheck.AnchorPoint = Vector2.new(0.5, 0)
tradeCheck.Position = UDim2.new(0.5, 0, 0, 15)
tradeCheck.BackgroundTransparency = 1
tradeCheck.Text = "✓"
tradeCheck.TextColor3 = Color3.fromRGB(78, 225, 157)
tradeCheck.TextSize = 38
tradeCheck.Font = Enum.Font.GothamBold
tradeCheck.TextTransparency = 1
tradeCheck.ZIndex = 202
tradeCheck.Parent = tradeResultCard

local tradeTitle = Instance.new("TextLabel")
tradeTitle.Size = UDim2.new(1, -24, 0, 28)
tradeTitle.Position = UDim2.new(0, 12, 0, 65)
tradeTitle.BackgroundTransparency = 1
tradeTitle.Text = "TRADE SUCCESSFUL!"
	playLocalTradeSuccess()
tradeTitle.TextColor3 = Color3.fromRGB(245, 245, 255)
tradeTitle.TextSize = 18
tradeTitle.Font = Enum.Font.GothamBold
tradeTitle.TextTransparency = 1
tradeTitle.ZIndex = 202
tradeTitle.Parent = tradeResultCard

local tradeSub = Instance.new("TextLabel")
tradeSub.Size = UDim2.new(1, -24, 0, 20)
tradeSub.Position = UDim2.new(0, 12, 0, 96)
tradeSub.BackgroundTransparency = 1
tradeSub.Text = "Local visual trade completed"
tradeSub.TextColor3 = Color3.fromRGB(145, 145, 170)
tradeSub.TextSize = 11
tradeSub.Font = Enum.Font.GothamMedium
tradeSub.TextTransparency = 1
tradeSub.ZIndex = 202
tradeSub.Parent = tradeResultCard

local function playLocalTradeSuccess()
	if tradeVisualBusy then return end
	tradeVisualBusy = true
	tradeOverlay.Visible = true
	tradeOverlay.BackgroundTransparency = 1
	tradeResultCard.BackgroundTransparency = 1
	tradeResultCard.Size = UDim2.fromOffset(270, 112)
	tradeResultStroke.Transparency = 1
	tradeCheck.TextTransparency = 1
	tradeCheck.Rotation = -18
	tradeTitle.TextTransparency = 1
	tradeSub.TextTransparency = 1

	zynxTween(tradeOverlay, {BackgroundTransparency = 0.28}, 0.22, Enum.EasingStyle.Quad)
	zynxTween(tradeResultCard, {
		BackgroundTransparency = 0,
		Size = UDim2.fromOffset(300, 132)
	}, 0.34, Enum.EasingStyle.Back)
	zynxTween(tradeResultStroke, {Transparency = 0}, 0.28, Enum.EasingStyle.Quad)

	task.delay(0.10, function()
		zynxTween(tradeCheck, {TextTransparency = 0, Rotation = 0}, 0.28, Enum.EasingStyle.Back)
	end)
	task.delay(0.18, function()
		zynxTween(tradeTitle, {TextTransparency = 0}, 0.22, Enum.EasingStyle.Quad)
	end)
	task.delay(0.25, function()
		zynxTween(tradeSub, {TextTransparency = 0}, 0.22, Enum.EasingStyle.Quad)
	end)

	task.delay(1.55, function()
		zynxTween(tradeCheck, {TextTransparency = 1}, 0.20, Enum.EasingStyle.Quad)
		zynxTween(tradeTitle, {TextTransparency = 1}, 0.20, Enum.EasingStyle.Quad)
		zynxTween(tradeSub, {TextTransparency = 1}, 0.20, Enum.EasingStyle.Quad)
		zynxTween(tradeResultStroke, {Transparency = 1}, 0.22, Enum.EasingStyle.Quad)
		zynxTween(tradeResultCard, {
			BackgroundTransparency = 1,
			Size = UDim2.fromOffset(278, 116)
		}, 0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.In)
		zynxTween(tradeOverlay, {BackgroundTransparency = 1}, 0.26, Enum.EasingStyle.Quad)

		task.delay(0.29, function()
			tradeOverlay.Visible = false
			tradeVisualBusy = false
		end)
	end)
end
...
spawnedPlayers = spawnedPlayers or {}
